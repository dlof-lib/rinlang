...
// الممكن جمعها بأمان دفعة واحدة (multi-error + recovery، انظر Parser::parseCollectingDiagnostics)
// بدل التوقف عند أول خطأ. راجع docs/ERROR_SYSTEM.md، الأقسام 22/23/26/27.
int runCheck(const std::string& path, rin::diag::OutputFormat fmt) {
    ...
    try {
        ...
        // خطأ Lexer (لا يزال يرمي مباشرة، لا مجال آمن لاستئناف lexing بعد رمز غير صالح)
