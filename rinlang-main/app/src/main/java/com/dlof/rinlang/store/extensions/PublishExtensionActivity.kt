...
class PublishExtensionActivity : BaseConnectivityActivity() {
    ...
    private lateinit var progressGate: ProgressBar
    private lateinit var containerLocked: View
    private lateinit var containerForm: View
    private lateinit var txtLockedMessage: TextView
    ...
    /** عدد متابعي الحساب الحالي، يُحمَّل مرة واحدة عند فتح الشاشة ويُعاد التحقق منه دفاعاً
     *  بعمق عند الضغط على "نشر" (راجع [PublishPolicy.validateFollowerRequirement]). */
    private var currentFollowerCount: Long = 0L
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressGate = findViewById(R.id.progressExtPublishGate)
        containerLocked = findViewById(R.id.containerExtPublishLocked)
        containerForm = findViewById(R.id.containerExtPublishForm)
        txtLockedMessage = findViewById(R.id.txtExtPublishLockedMessage)

        findViewById<View>(R.id.btnExtPublishLockedBack).setOnClickListener { finish() }

        edtVersion.setText("1.0.0")

        // قيد المتابعين (راجع PublishPolicy.MIN_FOLLOWERS_TO_PUBLISH_EXTENSION): نتحقق أولاً
        // من عدد متابعي الحساب قبل عرض نموذج النشر أصلاً، بدل السماح بتعبئته ثم رفضه لاحقاً.
        if (!isOnline()) {
            progressGate.visibility = View.GONE
            showOfflineOverlay()
            return
        }
        AuthRepository.fetchProfile(uid) { profile ->
            progressGate.visibility = View.GONE
            currentFollowerCount = profile?.subscriberCount ?: 0L
            val followerCheck = PublishPolicy.validateFollowerRequirement(currentFollowerCount)
            if (followerCheck is PublishPolicy.PolicyResult.Denied) {
                containerLocked.visibility = View.VISIBLE
                containerForm.visibility = View.GONE
                txtLockedMessage.text = followerCheck.message
                return@fetchProfile
            }

            containerLocked.visibility = View.GONE
    ...
    private fun attemptPublish(uid: String) {
        ...
        var denied = false
        denyOrElse(PublishPolicy.validateFollowerRequirement(currentFollowerCount)) { denied = true }
        if (!denied) denyOrElse(PublishPolicy.validateName(name.lowercase().replace(" ", "-"))) { denied = true }
