...
object ProjectManager {
    ...
    /**
     * يفكّ ضغط أرشيف ZIP من [zipBytes] مباشرة (بلا Uri/ContentResolver) داخل مجلد المشروع،
     * بنفس منطق الأمان في [importZipFromUri] (يرفض أي عنصر يحاول الخروج خارج مجلد المشروع).
     * يُستخدَم لتنزيل مشروع نُشر في "معرض مشاريع Rin"
     * ([com.dlof.rinlang.store.projects.RinProjectRepository]) وفكّه محلياً بعد فكّ ترميز
     * base64 الوارد من Realtime Database، دون الحاجة لكتابة الأرشيف كملف مؤقت أولاً.
     */
    fun importProjectZipBytes(project: Project, zipBytes: ByteArray): Int {
        val projectRoot = project.dir.canonicalFile
        var extractedCount = 0

        ZipInputStream(java.io.ByteArrayInputStream(zipBytes)).use { zip ->
            var entry: ZipEntry? = zip.nextEntry
            while (entry != null) {
                val safeRelPath = sanitizeZipEntryPath(entry.name)
                if (safeRelPath != null) {
