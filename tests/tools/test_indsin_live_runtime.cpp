// tools/test_indsin_live_runtime.cpp — Phase 1 real-execution tests for Live Preview.
//
// These prove that indsin::runColdPipeline / runColdPipelineWithRuntime / runColdPipelineForContainer
...
//
// Build & run (same recipe as the other tools/test_indsin_*.cpp files):
//   cd app/src/main/cpp
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_live_runtime.cpp rin_lexer.cpp \
...
//       diagnostics/source_manager.cpp -lz -o test_indsin_live_runtime
//   ./test_indsin_live_runtime
#include "rin_lexer.h"
...
indsin::StrandPtr findByName(const indsin::StrandPtr& s, const std::string& name) {
    if (!s) return nullptr;
...
std::string attr(const indsin::StrandPtr& s, const std::string& key) {
    if (!s) return "";
    for (auto& a : s->attrs) {
...
int main() {
    ...
    // cell actually displayed and mutated through the full Indsin pipeline.)
    {
        std::string src = R"(
            let count = 0;
            while (count < 5) {
    ...
    // ---- 5. `warp` is a real global now: mutated by a real function via a real onTap dispatch ----
    {
        ...
        if (r.ok) {
            ...
            bool allGood = true;
            rin::Interpreter interp; // one persistent Interpreter across all 5 taps -- see IndsinSession
            for (int tap = 0; tap < 5; tap++) {
                std::unordered_map<std::string, rin::Value> globals;
                for (auto& kv : r.warp.cells) globals[kv.first] = indsin::indsinValueToRin(kv.second);
                ...
                for (auto& kv : globals) r.warp.set(kv.first, indsin::rinValueToIndsin(kv.second));
    ...
    // ---- 8. container-scoped pipeline: real execution scoped to the container's own body ----
    {
        ...
        auto r = indsin::runColdPipelineForContainer(src, "card");
