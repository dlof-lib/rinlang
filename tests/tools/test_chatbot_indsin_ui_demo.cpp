// tools/test_chatbot_indsin_ui_demo.cpp — verifies examples/chatbot_indsin_ui_demo.rin's Indsin UI
// half (warp bubbles + TextArea/placeholder + send Button + onTap) actually parses, lays out,
// and reacts to a real dispatchTap the same way tools/test_indsin_button.cpp verifies plain
// counters -- no engine changes, just proof the manual pattern the user asked for really works.
//
// Build (from app/src/main/cpp):
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_chatbot_indsin_ui_demo.cpp rin_lexer.cpp \
...
// Only the Indsin (@view/warp/fun) half of the demo -- the trailing container.chatbot half is
...
int main() {
    std::cout << "-- chatbot Indsin UI demo: parses + lays out --\n";
    auto r = indsin::runColdPipeline(kSource);
    CHECK(r.ok, "parses");
    ...
    r.warp.set("draftText", indsin::Value::txt("مرحباً يا بوت"));
    ...
    r.warp.set("draftText", indsin::Value::txt("سؤال ثاني"));
    auto tap2 = indsin::dispatchTap(r.fabric, r.warp, r.program, cx, cy);
