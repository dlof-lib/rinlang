// tools/test_indsin_button.cpp — tests for Rin Indsin's Button component library: variants
...
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_button.cpp rin_lexer.cpp rin_parser.cpp \
//       diagnostics/diagnostic.cpp diagnostics/diagnostic_engine.cpp \
//       diagnostics/diagnostic_renderer.cpp diagnostics/source_manager.cpp -o test_indsin_button
...
// runColdPipeline only builds the Fabric tree; it does not lay it out (indsinc.cpp calls
// indsin::Indsin::layout separately -- see its main()). Every test here needs real settled
// geometry, so this helper does what indsinc.cpp does before any assertion runs.
...
.end/view
        ...
        if (r.ok) {
            ...
            CHECK(hmed == 44, "medium height == 44 (matches the pre-Indsin hardcoded default exactly)");
        }
    }

    // 2. No size= at all still measures exactly like before Indsin's Button library existed -----
    {
...
.end/view
        ...
        if (r.ok) {
            ...
            CHECK(indsin::resolveButtonTreatment(*b) == indsin::ButtonTreatment::FILL, "variant defaults to fill");
            CHECK(indsin::resolveState(*b) == indsin::StrandState::NORMAL, "state defaults to normal");
        }
    }
...
.end/view
        ...
        if (r.ok) {
            ...
            auto tapLive = indsin::dispatchTap(r.fabric, r.warp, r.program, cx, cy);
            ...
            auto tapDead = indsin::dispatchTap(r.fabric, r.warp, r.program, dx, dy);
...
.end/view
        ...
        if (r.ok) {
            ...
            CHECK(indsin::accessibleRole(plain->kind) == "button", "Button's accessible role is 'button'");
            CHECK(indsin::accessibleName(*plain) == "Save", "accessible name defaults to the visible label");
            CHECK(indsin::accessibleName(*described) == "Save the current document",
...
@view.Button=b label="X"; tone="danger"; variant="outline"; .end/view
        ...
        if (r.ok) {
            CHECK(indsin::resolveButtonTreatment(*r.fabric) == indsin::ButtonTreatment::OUTLINE, "variant=outline resolves");
            indsin::Color danger; indsin::resolveSemanticColor("danger", danger);
            indsin::Color resolved = indsin::resolveColor(r.fabric);
