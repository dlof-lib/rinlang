...
/**
 * Bridges the code editor ([MainActivity]) and the Live Preview screen ([IndsinPreviewActivity])
 * to a single native [RinEngine.IndsinSession] that stays alive for as long as the preview is
 ...
 * [IndsinPreviewActivity]. Rather than shuttling the full source text through Intents/extras (which
 ...
 * the session already produced a frame (e.g. [IndsinPreviewActivity] recreated by a rotation) is
 * synced immediately from the cache instead of racing the background thread for a fresh one.
 */
object IndsinPreviewManager {
    ...
    private val worker = Executors.newSingleThreadExecutor { r -> Thread(r, "indsin-preview-session") }
    private val mainHandler = Handler(Looper.getMainLooper())

    @Volatile private var session: RinEngine.IndsinSession? = null
     ...
     * see [RinEngine.IndsinSession.setViewport]. Kept here (not just pushed once) so a brand-new
     * session created by [start] — which the native side always boots at its own 844px default
     * (see rin_indsin_session_set_viewport's doc comment) — is immediately corrected to the real
    ...
    fun start(source: String, rootWidth: Int = this.rootWidth) {
        ...
        worker.execute {
            session?.close()
            val fresh = RinEngine.IndsinSession(source, rootWidth)
     ...
     * / Tooltip clamping — see [RinEngine.IndsinSession.setViewport]) and immediately re-delivers a
     ...
     * [RinEngine.IndsinSession.updateSource]. No-ops silently if no session is running yet.
     */
    ...
    /** Dispatches a long-press at ([x], [y]); see [RinEngine.IndsinSession.longPress]. */
    ...
    /** Dispatches a double-tap at ([x], [y]); see [RinEngine.IndsinSession.doubleTap]. */
    ...
    /** Dispatches a hover enter/exit at ([x], [y]); see [RinEngine.IndsinSession.hover]. */
