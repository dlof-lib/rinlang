...
/**
 * Rin's Live Preview screen for the Indsintime rendering engine ("Mirror Indsin"): shows a
 * `@view.<Kind>=name ... .end/view` tree exactly as the native C++ Indsin engine laid it out,
 * inside a phone-shaped device frame, and keeps it live — every keystroke back in [MainActivity]
 * hot-updates the very same [IndsinPreviewManager] session (preserving Warp state, e.g. a tapped
...
class IndsinPreviewActivity : AppCompatActivity(), IndsinPreviewManager.Listener {
    ...
    companion object {
        ...
        const val EXTRA_CODE = "indsin_preview_code"
        ...
        const val EXTRA_FILE_NAME = "indsin_preview_file_name"
    ...
    private lateinit var fabricView: IndsinFabricView
     ...
     * actually switch pages instead of [IndsinFabricView.onNavigate] firing into nothing. */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fabricView = findViewById(R.id.indsinFabricView)
        txtTitle = findViewById(R.id.txtIndsinTitle)
        previewSurface = findViewById(R.id.indsinHScroll)
        errorBanner = findViewById(R.id.indsinErrorBanner)
        txtError = findViewById(R.id.txtIndsinError)
        inspectorPanel = findViewById(R.id.indsinInspectorPanel)
        txtInspectorTitle = findViewById(R.id.txtInspectorTitle)
        txtInspectorBody = findViewById(R.id.txtInspectorBody)
        txtStats = findViewById(R.id.txtIndsinStats)
        txtZoom = findViewById(R.id.txtIndsinZoom)
        btnDevice = findViewById(R.id.btnIndsinDevice)
        btnGrid = findViewById(R.id.btnIndsinGrid)

        val btnClose: ImageButton = findViewById(R.id.btnIndsinClose)
        val btnZoomOut: ImageButton = findViewById(R.id.btnIndsinZoomOut)
        val btnZoomIn: ImageButton = findViewById(R.id.btnIndsinZoomIn)
        val btnRefresh: ImageButton = findViewById(R.id.btnIndsinRefresh)
        ...
        fabricView.onTap = { x, y -> IndsinPreviewManager.tap(x, y) }
        ...
        fabricView.onLongPressHandler = { x, y -> IndsinPreviewManager.longPress(x, y) }
        fabricView.onDoubleTapHandler = { x, y -> IndsinPreviewManager.doubleTap(x, y) }
        fabricView.onHoverHandler = { x, y, entering -> IndsinPreviewManager.hover(x, y, entering) }
        ...
        fabricView.onOpenUrl = { url ->
            try {
                ...
                Toast.makeText(this, getString(R.string.indsin_link_open_failed, url), Toast.LENGTH_SHORT).show()
            }
        }
        ...
        fabricView.onZoomChanged = { z -> txtZoom.text = getString(R.string.indsin_zoom_percent_format, (z * 100).toInt()) }

        // Overlay Engine (Dialog centering / Tooltip clamping — see [IndsinPreviewManager.setViewport]):
        ...
        previewSurface.addOnLayoutChangeListener { _, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom ->
            ...
            if (heightPx > 0) {
                ...
                IndsinPreviewManager.setViewport(viewportDp)
            }
        }
        ...
        IndsinPreviewManager.attach(this)

        currentDeviceWidth = if (IndsinPreviewManager.isRunning) IndsinPreviewManager.rootWidth else DEFAULT_DEVICE_WIDTH
        btnDevice.text = getString(R.string.indsin_device_width_format, currentDeviceWidth)
        ...
        if (savedInstanceState == null) {
            ...
            if (incomingCode != null) {
                if (IndsinPreviewManager.isRunning) {
                    IndsinPreviewManager.start(incomingCode, currentDeviceWidth)
                } else {
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        inspectorPanel.visibility = View.GONE
        fabricView.clearInspection()
        IndsinPreviewManager.start(code, currentDeviceWidth)
    }
     ...
     * page we came from onto [pageStack] for system Back. Fired by [IndsinFabricView.onNavigate] —
    ...
    private fun navigateTo(target: String) {
        ...
        if (newSource == null) {
            Toast.makeText(this, getString(R.string.indsin_navigate_missing_toast, target), Toast.LENGTH_SHORT).show()
            return
        }

        pageStack.addLast(currentPageName to IndsinPreviewManager.lastSource)
        currentPageName = file.name
        txtTitle.text = currentPageName
        inspectorPanel.visibility = View.GONE
        fabricView.clearInspection()
        IndsinPreviewManager.start(newSource, currentDeviceWidth)
    }
    ...
    private fun navigateBack(): Boolean {
        ...
        inspectorPanel.visibility = View.GONE
        fabricView.clearInspection()
        IndsinPreviewManager.start(source, currentDeviceWidth)
        return true
    }
    ...
    // IndsinPreviewManager.tick() once per display frame while an enter transition is in progress,
    ...
    private val effectsFrameCallback = android.view.Choreographer.FrameCallback {
        ...
        IndsinPreviewManager.tick() // its result flows back through onFabricUpdated, which re-arms if still animating
    }
    ...
    private fun renderResult(resultJson: String, elapsedMs: Long) {
        ...
        if (result == null) {
            showError(getString(R.string.indsin_internal_error))
            return
        }
        ...
        if (fabric == null) {
            ...
            txtStats.text = getString(R.string.indsin_stats_error_only)
            return
        }
        ...
        txtStats.text = if (measured >= 0) {
            getString(R.string.indsin_stats_format, measured, elapsedMs, cache)
        } else {
            getString(R.string.indsin_stats_idle)
        }
    }
    ...
    private fun sanitizePreviewGeometry(node: JSONObject) {
        val viewport = IndsinPreviewManager.viewportHeight.coerceAtLeast(120)
    ...
    private fun formatError(result: JSONObject): String {
        ...
        val message = result.optString("error", getString(R.string.indsin_unknown_error))
        return if (line > 0) getString(R.string.indsin_error_line_format, line, message)
        else getString(R.string.indsin_error_format, message)
    }
    ...
    private fun showInspector(node: JSONObject?) {
        ...
        txtInspectorTitle.text = getString(R.string.indsin_inspector_title_format, kind, name, line)
    ...
    private fun setZoom(value: Float) {
        ...
        txtZoom.text = getString(R.string.indsin_zoom_percent_format, pct)
    }

    private fun restartSession(showToast: Boolean) {
        val source = IndsinPreviewManager.lastSource
        IndsinPreviewManager.start(source, currentDeviceWidth)
        inspectorPanel.visibility = View.GONE
        fabricView.clearInspection()
        if (showToast) {
            Toast.makeText(this, getString(R.string.indsin_session_restarted_toast), Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun showDeviceMenu(anchor: View) {
        ...
        popup.menu.add(0, -1, 0, getString(R.string.indsin_device_fit_screen))
        DEVICE_WIDTHS.forEachIndexed { index, width ->
            popup.menu.add(0, index, index + 1, getString(R.string.indsin_device_width_format, width))
        }
        popup.setOnMenuItemClickListener { item ->
            ...
            if (width != null && width != currentDeviceWidth) {
                ...
                btnDevice.text = getString(R.string.indsin_device_width_format, width)
                ...
                Toast.makeText(this, getString(R.string.indsin_device_changed_toast, width), Toast.LENGTH_SHORT).show()
            }
            true
        }
        popup.show()
