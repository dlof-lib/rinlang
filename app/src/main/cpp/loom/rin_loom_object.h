...
// once more directly from the parsed AST for Loomtime's benefit, exactly like
...
// لكل سمة @view أخرى.
inline std::vector<std::pair<std::string, Value>> loomExtractContainerFields(const std::vector<rin::StmtPtr>& body, WarpScope& warp) {
    std::vector<std::pair<std::string, Value>> fields;
    for (auto& st : body) {
...
inline void loomCollectGroupRows(const std::vector<rin::StmtPtr>& body, WarpScope& warp,
    ...
    for (auto& st : body) {
        if (auto g = std::dynamic_pointer_cast<rin::ContainerGroupStmt>(st)) {
            loomCollectGroupRows(g->body, warp, out);
        } else if (auto v = std::dynamic_pointer_cast<rin::VolumeStmt>(st)) {
            loomCollectGroupRows(v->body, warp, out);
            ...
            out.push_back({c->name.empty() ? std::string("#") + std::to_string(out.size()) : c->name,
                            loomExtractContainerFields(c->body, warp)});
        }
    }
}

// يُستدعى من الأنبوب البارد/الساخن (rin_loom_pipeline.h)، بنفس مكان استدعاء
...
inline void registerGroupsFromProgram(const std::vector<rin::StmtPtr>& program, WarpScope& warp) {
    ...
    for (auto& stmt : program) {
        ...
        loomCollectGroupRows(*body, warp, rows);
