// indsin/rin_indsin_overlay.h — Overlay Engine.
//
...
// Tooltip's measureTooltip call in rin_indsin_layout.h still run exactly as before, and still
...
//      the tree parent-then-children, siblings in source order — see rin_indsin_paint.h). A Dialog
...
//      contribution entirely (see the tail of Indsin::layout() in rin_indsin_layout.h), and
...
//      (see the dispatchTapWithOverlay wrapper in rin_indsin_needle.h): a tap outside an open modal
...
// (Constraints{0, rootWidth, 0, 1e9} in rin_indsin_c_api.cpp — deliberately unbounded, since the
// rest of the Indsin lays out a scrollable design-time canvas, not a fixed screen). A Dialog
...
#include "rin_indsin_layout.h" // alignOffset() (free function) + Indsin::translate() (member) — reused, not reinvented
#include <string>
#include <vector>
#include <algorithm>
...
// isOverlayStrand() itself lives in rin_indsin_layout.h (Indsin::layout()'s own tail needs it to
...
// is paint order" convention the rest of the Indsin already uses, just applied to this second
...
// Strand's id (see deriveId in rin_indsin_strand.h), just looked up at runtime instead of parse time.
...
// Runs after Indsin::layout(root, ...) has already measured the whole fabric (so every open
...
// mutation stays the one place Indsin::translate already owns.
inline OverlayLayer buildOverlayLayer(Indsin& engine, const StrandPtr& root,
