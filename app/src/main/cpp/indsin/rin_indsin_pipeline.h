// indsin/rin_indsin_pipeline.h — IndsintimeRuntime: ties the real rin::Lexer/rin::Parser to the
// Indsintime engine. This is the "IndsintimeRuntime sibling" described in the architecture doc's
...
// small Indsintime Value type WarpScope stores. Deliberately not shared with rin_indsin_needle.h's
// identically-shaped rinValueToIndsin(): that header is a *higher* layer built on top of this one
// (rin_indsin_c_api.cpp includes both), and pulling it in here just to reuse five lines would
// invert that dependency for no benefit.
inline Value runtimeValueToIndsin(const rin::Value& v) {
    switch (v.type) {
...
struct PipelineResult {
    ...
    // roots the pipeline itself cares about) so Needle (rin_indsin_needle.h) can find a top-level
    ...
    // evaluator in rin_indsin_eval.h.
...
//
// Editor -> Lexer -> Parser -> Runtime Execution -> Runtime State -> Indsin Fabric -> Layout -> Render.
//
...
// functions -- exactly like running the same source as a normal Rin program, before Indsin ever
// looks at it. Indsin's only job afterwards is to read back the *result* of that real execution
...
// persistent Interpreter (see IndsinSession in rin_indsin_c_api.cpp) so state (containers, chat
    ...
    try {
        ...
        for (auto& stmt : program) {
            if (auto w = std::dynamic_pointer_cast<rin::WarpStmt>(stmt)) {
                ...
                Value v = (it != liveGlobals.end()) ? runtimeValueToIndsin(it->second)
                                                     : evalAttrExpr(w->initializer, result.warp, nullptr);
                result.warp.set(w->name, v);
            }
        }
        // Rin Indsin: register any @theme=... blocks (Pattern Book) before the Fabric is built,
...
// pre-existing call site (rin_indsin_render_json, tests, tools...) keeps compiling and behaving the
...
// *inside* the named @container (not the top-level program). This is what makes Rin Indsin actually
    ...
    try {
        ...
        for (auto& stmt : *body) {
            if (auto w = std::dynamic_pointer_cast<rin::WarpStmt>(stmt)) {
                ...
                Value v = (it != liveContainerVars.end()) ? runtimeValueToIndsin(it->second)
                                                            : evalAttrExpr(w->initializer, result.warp, nullptr);
                result.warp.set(w->name, v);
            }
        }
