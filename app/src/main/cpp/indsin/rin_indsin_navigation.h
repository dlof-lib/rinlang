// indsin/rin_indsin_navigation.h — Navigation Manager (spec §19): a small, crash-safe history stack
// behind navigate()/back()/replace()/reload(). This owns *history bookkeeping* only -- it does
// not read a .rin file or swap the live Fabric itself, because Indsintime's rendering pipeline
// (rin_indsin_pipeline.h) is a pure `source -> Fabric` function with no notion of "the current
// page"; on Android that swap already happens one layer up (IndsinPreviewActivity/RinEngine.kt).
