// indsin/pitok_indsin_bridge.h — Phase 2 of the PITOK separation: feeds Indsintime's existing
...
//
// What this does NOT change: rin_indsin_layout.h, rin_indsin_paint.h, rin_indsin_needle.h all keep
// consuming the exact same indsin::Strand tree they always have. This file is only a second way
// to *produce* that tree — pitok::ViewNode -> indsin::Strand — that never touches rin::Lexer,
// rin::Parser, or rin::Interpreter. A source file whose UI is written in the @view/@loop dialect
// can now go: pitok::Lexer -> pitok::Parser -> (this file) -> indsin::Strand -> existing
// layout()/paint(), with zero Rin-general-purpose parsing anywhere on that path.
//
// Honest scope note (see pitok/README.md "ما لم يُنجَز بعد"): indsin::Strand::role is
// rin::UiRole and indsin::ResolvedAttr::rawExpr is rin::ExprPtr — both still literally *named*
...
// Mirrors indsin::evalAttrExpr (rin_indsin_eval.h) field-for-field, but walks pitok::Expr instead
...
// logic (rin_indsin_needle.h) needs no changes to handle PITOK-sourced Strands.
inline indsin::Value evalPitokExpr(const pitok::ExprPtr& e, const indsin::WarpScope& warp,
                                  std::vector<std::string>* readNames = nullptr) {
    if (!e) return indsin::Value::txt("");
    switch (e->kind) {
        case pitok::ExprKind::Number:
            return indsin::Value::num(e->numValue);
        case pitok::ExprKind::String:
            return indsin::Value::txt(e->strValue);
        case pitok::ExprKind::Identifier: {
            ...
            return indsin::Value::txt(e->name);
        }
        case pitok::ExprKind::Binary: {
            indsin::Value l = evalPitokExpr(e->left, warp, readNames);
            indsin::Value r = evalPitokExpr(e->right, warp, readNames);
            if (e->op == "+") {
                if (l.kind == indsin::Value::Kind::NUMBER && r.kind == indsin::Value::Kind::NUMBER)
                    return indsin::Value::num(l.number + r.number);
                return indsin::Value::txt(l.asString() + r.asString()); // '+' string concat, same as Rin
            }
            if (e->op == "-") return indsin::Value::num(l.asNumber() - r.asNumber());
            if (e->op == "*") return indsin::Value::num(l.asNumber() * r.asNumber());
            if (e->op == "/") return indsin::Value::num(r.asNumber() != 0 ? l.asNumber() / r.asNumber() : 0.0);
            return indsin::Value::txt(l.asString() + r.asString());
        }
        case pitok::ExprKind::Call: {
            ...
            os << ")";
            return indsin::Value::txt(os.str());
        }
    }
    return indsin::Value::txt("<expr>");
}

// Mirrors indsin::buildFabric (rin_indsin_strand.h) field-for-field for the @view/@loop dialect,
...
inline indsin::StrandPtr buildFabricFromPitok(const pitok::ViewNodePtr& node, indsin::WarpScope& warp,
                                             indsin::WarpSubscriptions& subs,
                                             const std::string& parentPath, int idx) {
    auto s = std::make_shared<indsin::Strand>();
    bool isLoop = (node->kind == "__loop__");
    bool isContainer = (node->kind == "__container__");
    // A Loop is the canvas -> COLUMN (implicit top-to-bottom root), matching Indsintime convention.
    ...
    if (isLoop) s->kind = indsin::StrandKind::COLUMN;
    else if (isContainer) s->kind = indsin::StrandKind::BOX;
    else s->kind = indsin::strandKindFromTag(node->kind);
    ...
    if (s->kind == indsin::StrandKind::CUSTOM) s->customTag = node->kind;
    s->name = node->name;
    s->mask = "";
    s->sourceLine = node->line;
    std::string myPath = parentPath + "/" + (node->name.empty() ? ("#" + std::to_string(idx)) : node->name);
    s->id = indsin::deriveId(parentPath, node->name, idx, s->kind);

    uint64_t attrHash = 1469598103934665603ULL;
    for (auto& a : node->attributes) {
        std::vector<std::string> reads;
        indsin::Value v = evalPitokExpr(a.value, warp, &reads);
        ...
        attrHash = indsin::fnv1a(a.key + "=" + v.asString(), attrHash);
    }
    ...
    // it into rin_indsin_needle.h's dispatch table yet (Needle today only recognizes onTap/etc.
    ...
    for (auto& b : node->bindings) {
        indsin::Value v = evalPitokExpr(b.action, warp, nullptr);
        std::string key = "on." + b.target + "." + b.event;
        s->attrs.push_back({key, nullptr, v});
        attrHash = indsin::fnv1a(key + "=" + v.asString(), attrHash);
    }

    uint64_t childHashAcc = 0; int i = 0;
    for (auto& c : node->children) {
        auto child = buildFabricFromPitok(c, warp, subs, myPath, i++);
        childHashAcc = indsin::fnv1a(std::to_string(child->contentHash), childHashAcc);
        s->children.push_back(child);
    }
    s->contentHash = indsin::fnv1a(indsin::strandKindName(s->kind), indsin::fnv1a(std::to_string(attrHash), childHashAcc));
    return s;
}

struct PitokPipelineResult {
    indsin::StrandPtr fabric;
    indsin::WarpScope warp;
    indsin::WarpSubscriptions subs;
    bool ok = false;
...
// see pitok/tools/pitok_parse.cpp for the standalone CLI equivalent) -> indsin::Strand ready for
// rin_indsin_layout.h's layout() and rin_indsin_paint.h's paint(). No rin::Lexer/Parser/Interpreter
...
inline PitokPipelineResult runPitokColdPipeline(const pitok::Program& prog) {
    ...
    try {
        for (auto& w : prog.warps) {
            indsin::Value v = evalPitokExpr(w.initValue, result.warp, nullptr);
