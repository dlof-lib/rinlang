// indsin/rin_indsin_actions.h — Indsin Action Engine: a single registry of named, Warp-cell-driven
// actions (increment/decrement/toggle/set/show/hide/enable/disable/select/unselect/focus/blur/
// play/pause/stop/seek/scrollTo/submit/reset/seq), used by Needle (rin_indsin_needle.h) as the
...
// rin_indsin_navigation.h and are wired into Needle separately, since they need a NavigationManager
...
class ActionRegistry {
public:
    // register(name, fn): the extensibility point spec §36 asks for -- a new Indsin Action never
...
inline void ActionRegistry::registerBuiltins() {
    ...
    // model belongs to the native media player, not Needle -- see docs/indsintime for the honest
