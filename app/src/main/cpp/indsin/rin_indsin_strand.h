// indsin/rin_indsin_strand.h — The Fabric (virtual scene graph) + Renderer Engine (AST -> Fabric).
// Consumes rin::ViewStmt directly (see the Indsintime extension in rin_ast.h) — no separate
...
enum class StrandKind {
    ...
    // Extended UI kinds required by rin_indsin_layout.h
    ...
    // docs/indsintime/RIN_INDSIN_TOKENS.md: `Container` already means a data/storage construct in
    ...
    // measureButton()/paintButton() wholesale (rin_indsin_layout.h/rin_indsin_paint.h) rather than
    ...
    // `@Object=name`) registered in indsin::objectLiteralRegistry() — the Indsin-side counterpart of
    // the console-only `view.print/object(...)` statement (rin_interpreter.cpp), now wired into
    // the actual Fabric/Indsin/Dye pipeline instead of stdout. Same naming note as BOX above: the
...
struct Strand {
    ...
    // true, margin-EXCLUDED box that paint.h and hit-testing (rin_indsin_needle.h) use — margin is
...
// this is how a runtime warp.set(...) finds exactly the Strands to re-resolve (see rin_indsin_shuttle.h).
...
//
// Scope note: this only runs on the cold-pipeline build path (see rin_indsin_pipeline.h's
...
// Mask queries for Indsin: the mask is a logical identity, not merely a styling attribute.
