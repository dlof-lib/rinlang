...
private:
    ...
    std::string emitCall(const std::shared_ptr<CallExpr>& c, Scope& sc) {
        ...
        static const std::unordered_set<std::string> natives = {
            ...
            // انظر rt_native_rinVersion/rt_native_rinEdition أعلاه في RUNTIME_HEADER —
            // ثابتان حرفيّان يُحدَّثان يدوياً مع rin_version.h، انظر docs/VERSIONING.md.
            "rinVersion","rinEdition"
        };
...
// رقمان حرفيّان ثابتان — ليسا مأخوذين من rin_version.h عبر #include عمداً: rinc.cpp يجب
// أن يبقى ملفاً واحداً قائماً بذاته يُبنى بـ `g++ -o rinc compiler/rinc.cpp` بلا أي ملف
// آخر من المشروع (انظر compiler/README.md)، فإضافة #include "rin_version.h" هنا تكسر
// هذا العقد. لذلك: عند رفع إصدار المحرّك في rin_version.h، حدِّث السطرين التاليين يدوياً
// ليطابقا RIN_VERSION_STRING/RIN_VERSION_EDITION (موثَّق أيضاً في docs/VERSIONING.md).
static Value rt_native_rinVersion(Value* a, int n) { (void)a; (void)n; return rt_str("1.0.0"); }
static Value rt_native_rinEdition(Value* a, int n) { (void)a; (void)n; return rt_str("2026"); }
