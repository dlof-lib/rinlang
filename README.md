...
| 🛍️ **متجر Rin للحزم** | تصفّح، تثبيت، تقييم، ونشر حزم المجتمع مباشرة من داخل التطبيق — بلا حاجة لأي أداة خارجية |
...
- [متجر Rin — تصفّح وتثبيت ونشر الحزم](#متجر-rin--تصفّح-وتثبيت-ونشر-الحزم)
...
// break / continue داخل الحلقات
let n = 0;
while (n < 10) {
    n = n + 1;
    if (n % 2 == 0) { continue; } // يتخطّى باقي الجسم ويعود لفحص الشرط
    if (n > 7) { break; }          // يخرج فوراً من الحلقة
    print n;
}
...
أبسط استخدام لـ `print` هو قيمة واحدة تليها فاصلة منقوطة، لكن العبارة تدعم مجموعة سمات
اختيارية (كل واحدة `key=value`، بأي ترتيب، بلا فاصلة قبلها) تجعلها أداة تسجيل/طباعة كاملة
لبناء أدوات سطر أوامر ولوجات احترافية بدون أي حاجة لدوال مساعدة إضافية:

```kotlin
// عدّة قيم مفصولة بفواصل + sep/end مخصّصين
print "x=", 1, "y=", 2 sep=" | " end="\n";      // x= | 1 | y= | 2

// if= : بوابة طباعة شرطية — لا يُقيَّم أي شيء (لا القيم ولا بقية السمات) عند false
let debugMode = false;
print "قيمة تصحيح حسّاسة:", someValue if=debugMode;

// level= : يضيف رمز تصنيف يلتقطه كونسول تطبيق الأندرويد ليُلوّن السطر تلقائياً
print "تم الاتصال بنجاح" level="success";  // ✅
print "المساحة التخزينية شارفت على الامتلاء" level="warn";  // ⚠️
print "فشل الاتصال بالخادم" level="error"; // ❌
print "قيمة x الحالية =", x level="debug" label="TRACE"; // 🐞 [TRACE] ...

// repeat= : تكرار نفس السطر — مفيد لفواصل/بانرات console
print "-" repeat=40 end="";
print "";

// pretty= : تهيئة مصفوفة/قاموس متعددة الأسطر بدل السطر المضغوط المعتاد
let data = {name: "Rin", tags: [1, 2, 3]};
print data;               // {"name": "Rin", "tags": [1, 2, 3]}
print data pretty=true;   // تهيئة متعدّدة الأسطر بمسافات بادئة

// upper=/lower= : تحويل حالة الأحرف على الناتج النهائي (لا يمكن استخدامهما معاً)
print "shout" upper=true; // SHOUT

// width=/align= : حشو ومحاذاة (مفيد لجداول/أعمدة console) — align بلا width خطأ صريح
print "name" width=12 end="|";       // محاذاة يسار افتراضياً
print "42" width=12 align="right" end="|";
print "hi" width=12 align="center" end="|";
```

| السمة | النوع المتوقّع | الافتراضي | الوصف |
|---|---|---|---|
| `sep` | نص | `" "` | الفاصل بين القيم عند تعدّدها. |
| `end` | نص | `"\n"` | ما يُطبع بعد آخر قيمة (بدل السطر الجديد). |
| `if` | أي قيمة (تُقيَّم truthy/falsy) | — | بوابة تنفيذ: عند falsy لا يُقيَّم أي شيء إطلاقاً. |
| `level` | نص: `info`/`success`/`warn`/`error`/`debug` | — | يضيف رمز تصنيف يميّز السطر لوناً/أيقونة في كونسول التطبيق. |
| `label` | نص | — | وسم مخصّص يُطبع كـ `[TAG]` بعد رمز `level` (أو في البداية إن غاب). |
| `repeat` | رقم صحيح ≥ 0 | `1` | يكرر السطر كاملاً n مرة؛ `0` لا يطبع شيئاً. |
| `pretty` | منطقي | `false` | تهيئة متعددة الأسطر لأي مصفوفة/قاموس ضمن القيم. |
| `upper` / `lower` | منطقي | `false` | تحويل حالة الأحرف على الناتج المُجمَّع بالكامل. |
| `width` | رقم | — | حشو الناتج (بمسافات) لعرض أدنى؛ لا يُقصَّر أبداً إن كان المحتوى أطول. |
| `align` | نص: `left`/`right`/`center` | `left` | جهة الحشو، يتطلّب `width` معه. |
...
### وسم الإغلاق `.end/...` — دليل كامل

كل كتلة (`container`, `Containers.Group`, `Volume`, `Section`, `Translations`, `@view...`، وباقي
المفاهيم أدناه) تُغلَق بوسم `.end/الكلمة` يطابق كلمة الفتح تماماً — وإلا خطأ صريح وقت التحليل
النحوي **يذكر رقم سطر الفتح نفسه**، فلا تحتاج للبحث يدوياً بين عشرات أسطر `.end/...` المتشابهة
في ملف كبير:

```kotlin
@container=Alpha
    text a = "1";
.end/Volume   // ❌ خطأ: "Closing tag '.end/Volume' does not match the opening '@container' opened at line 1"
```

**اختصار `.end;`** — يغلق الكتلة الحالية أياً كان نوعها/اسمها بلا حاجة لتكرار كتابتهما، مفيد
خصوصاً عند تعشيش عميق (`container` داخل `Containers.Group` داخل `Volume`...) حيث يصبح تكرار
`.end/container.pipe` في كل مستوى متعباً ومصدر أخطاء نسخ/لصق. الصحة البنيوية (أي كتلة تُغلق
فعلياً) تبقى مضمونة بالكامل بمعزل عن هذا الاختصار — يحدّدها ترتيب النزول التكراري في المحلل
النحوي نفسه، لا ما يكتبه المبرمج بعد `.end`:

```kotlin
@Volume=V1
    @container=inner
        text a = "1";
    .end;      // يغلق @container=inner
.end;          // يغلق @Volume=V1
```

**تحقق اختياري من الاسم `.end/tag=name`** — طبقة أمان إضافية اختيارية للحاويات المتداخلة الكبيرة:
اكتب اسم الحاوية بعد كلمة الإغلاق، ويتحقق المحلل النحوي فعلياً (وليس مجرد تعليق قد ينسى المبرمج
تحديثه) أنه مطابق حرفياً لاسم `@tag=name` الذي فتحها:

```kotlin
@container=Alpha
    text a = "1";
.end/container=Beta   // ❌ خطأ: لا يطابق اسم الفتح 'Alpha' (فُتح في السطر 1)

@container=Alpha
    text a = "1";
.end/container=Alpha  // ✅ يطابق تماماً
```

الشكلان (المطوَّل مع/بلا اسم، والمختصر `.end;`) متوافقان بالكامل مع كل الملفات القديمة — لا حاجة
لأي تعديل على كود موجود مسبقاً.
...
| `print` | `print expr [, expr...] [attr=value...];` | طباعة/عرض أي قيمة أو نص، بخيارات احترافية كاملة (sep/end/if/level/label/repeat/pretty/upper/lower/width/align) — انظر قسم «`print` — دليل كامل» أعلاه. |
...
## متجر Rin — تصفّح وتثبيت ونشر الحزم

<img alt="Store" src="https://img.shields.io/badge/متجر%20Rin-تصفّح%20وتثبيت%20وتقييم-7C5CFF?style=for-the-badge&logo=googleplay&logoColor=white" />

فوق نظام `@import` والمكتبات القياسية الخمس، يوفّر التطبيق **متجراً كاملاً للحزم** (`RinStoreActivity`
وشاشاتها الفرعية) يشبه من حيث التجربة متاجر الحزم المعروفة (npm/pub.dev) لكن داخل الجوال مباشرة، بلا
حاجة لأي أداة سطر أوامر:

<details>
<summary><b>تفاصيل شاشات المتجر (اضغط للتوسيع)</b></summary>

#### 🔍 شاشة التصفّح (Browse)

- **بحث فوري** باسم الحزمة أو وصفها أو اسم الناشر أثناء الكتابة.
- **تصنيفات ديناميكية** (Chips) تُبنى تلقائياً من تصنيفات الحزم المنشورة فعلياً — لا تصنيف ثابت مبرمَج مسبقاً.
- **فرز** حسب الأحدث / الأعلى تقييماً / الأكثر تحميلاً / الاسم أبجدياً.
- **تحميل هيكلي (Skeleton/Shimmer)** أثناء أول جلب للبيانات بدل شاشة فارغة أو مؤشر تحميل دائري بسيط.
- كل بطاقة حزمة تعرض: الاسم + الناشر (مع شارة "ناشر موثّق" 🔵 عند التأهل)، الوصف، التبعيات
  المطلوبة (إن وُجدت) على هيئة وسم واضح، تقييم بالنجوم، وعدد التنزيلات.

#### 📄 شاشة تفاصيل الحزمة (Package Detail)

شاشة مستقلة تُفتَح عند الضغط على أي بطاقة، بتصميم شبيه بصفحة مستودع على GitHub:

- رأس (Hero) يجمع اسم الحزمة، وصفها، صورة/حرف الناشر، وشريط إحصائيات موحّد (الإصدار • الترخيص •
  عدد التنزيلات • التقييم).
- عرض **README.md** الخاص بالحزمة مُصيَّراً (Markdown → HTML خفيف عبر `MarkdownLite.kt`) بدل نص خام.
- عرض نص **الترخيص** كاملاً.
- **قائمة كل ملفات الحزمة** مع حجم كل ملف.
- زر **تثبيت** واحد يُعيد النتيجة لشاشة المتجر التي تتولى التحقق من التبعيات ثم التثبيت الفعلي.

#### 📦 التثبيت والتحقق من التبعيات

قبل تثبيت أي حزمة، يتحقق `DependencyResolver` من كل تبعياتها المُعلَنة داخل `PackageManifest`:

- إن كانت كل التبعيات متوفرة ومطابقة للإصدار المطلوب → تثبيت فوري بلا أي تدخّل من المستخدم.
- إن كانت بعض التبعيات ناقصة أو بإصدار مختلف → حوار يعرض التفاصيل بدقة، مع خيار **"تثبيت مع التبعيات
  المتوفرة"** (يحلّها تلقائياً بعمق واحد من كتالوج المتجر نفسه) أو **"تثبيت على أي حال"**.
- عند نجاح التثبيت تُدرَج أسطر `@import "lib/...";` الجاهزة مباشرة في محرر الكود عند موضع المؤشر —
  بلا أي كتابة يدوية من المستخدم.

#### ⭐ التقييم والإبلاغ

- تقييم الحزمة من 1 إلى 5 نجوم عبر حوار مخصّص، بتسمية جودة (ضعيف → ممتاز) تتحدّث فوراً مع كل نجمة.
- الإبلاغ عن حزمة مخالفة (محتوى مسيء، كود ضار، انتهاك ترخيص، معلومات مضلِّلة...) بقائمة أسباب جاهزة.
- كلاهما يتطلّب تسجيل دخول لمنع التقييمات/البلاغات الوهمية.

#### 🚀 نشر حزمة (Publish)

أي مكتبة يكتبها المستخدم في شاشة "المكتبات" يمكن نشرها للمتجر مباشرة عبر `PublishPackageActivity`:

- توليد تلقائي لملفي `README.md` و `LICENSE` إن لم يرفع المستخدم نسخته الخاصة.
- ضغط كل الملفات (الكود + الأصول الاختيارية كصور/أيقونات) في حزمة `.og.rinsdk` واحدة.
- تحقّق من تفرّد اسم الحزمة قبل النشر لمنع التعارض مع حزم موجودة.

#### 👤 شاشة "حسابي" وشارة "ناشر موثّق"

- عرض/تعديل الملف الشخصي (الاسم، النبذة، الصورة).
- قسم **"حزمي المنشورة"**: كل الحزم التي نشرها المستخدم مع إمكانية حذفها نهائياً.
- شارة **"ناشر موثّق"** 🔵 تُمنح تلقائياً (بلا تدخّل بشري) لأي ناشر لديه حزمة واحدة على الأقل تجاوزت
  1000 تنزيل بتقييم 4 نجوم فأعلى — محسوبة بالكامل من طرف العميل (`PublisherBadgeUtils.kt`) دون أي
  طلب شبكة إضافي عند تحميل قائمة المتجر.

</details>

**الملفات المرتبطة:** `app/src/main/java/com/dlof/rinlang/store/` (منطق الشاشات)، و
`app/src/main/res/layout/*store*`, `*package*` (تخطيطات الواجهة)، و `firebase/database.rules.json`
(قواعد الوصول للبيانات على الخادم).
