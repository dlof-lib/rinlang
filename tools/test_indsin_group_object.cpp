// tools/test_indsin_group_object.cpp — §21b: Containers.Group/Volume as a data source for the
...
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_group_object.cpp rin_lexer.cpp \
//       rin_parser.cpp rin_interpreter.cpp rin_http.cpp rin_make.cpp \
//       diagnostics/diagnostic.cpp diagnostics/diagnostic_engine.cpp \
//       diagnostics/diagnostic_renderer.cpp diagnostics/source_manager.cpp -lz -o test_indsin_group_object
...
static void collectTexts(const indsin::StrandPtr& s, std::vector<std::string>& out) {
    if (!s) return;
    if (s->kind == indsin::StrandKind::TEXT) out.push_back(s->attrStr("text", ""));
