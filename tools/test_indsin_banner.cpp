// tools/test_indsin_banner.cpp — tests for Rin Indsin's Banner (§13/§14): title=/message=
...
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_banner.cpp rin_lexer.cpp rin_parser.cpp \
//       rin_interpreter.cpp rin_http.cpp diagnostics/diagnostic.cpp diagnostics/diagnostic_engine.cpp \
//       diagnostics/diagnostic_renderer.cpp diagnostics/source_manager.cpp -lz -o test_indsin_banner
...
static void layoutAt(indsin::PipelineResult& r, double width) {
...
.end/view
        ...
        if (r.ok) {
            ...
            // The Fabric's *attribute* updates immediately (mirroring rin_indsin_c_api.cpp's
            ...
            // itself, so recomputeHashes() (already provided by rin_indsin_strand.h specifically for
            ...
            // this test surfaced -- rin_indsin_c_api.cpp's rin_indsin_session_tap() calls
            ...
            // e.g. indsin_showcase.rin's counter text too) may not visibly resize through the real
            // session API today. Flagged in docs/indsintime/RIN_INDSIN_TOKENS.md; not silently patched
            // here since c_api.cpp is outside this slice's tested surface.
            for (auto& a : banner->attrs) {
                if (a.rawExpr) a.value = indsin::evalAttrExpr(a.rawExpr, r.warp, nullptr);
            }
            indsin::recomputeHashes(r.fabric); // must recompute from the ROOT: layout()'s cache check
