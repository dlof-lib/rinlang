// tools/test_indsin_overlay.cpp — tests for the Overlay Engine (rin_indsin_overlay.h): a real
// z-layer for Dialog/anchored-Tooltip, a scrim for modal Dialogs, and hit-test event blocking.
// This is the follow-up to test_indsin_missing_components.cpp's honest scope note ("Dialog/Tooltip
...
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_overlay.cpp rin_lexer.cpp \
...
//       diagnostics/source_manager.cpp -lz -o test_indsin_overlay
...
// rin_indsin_c_api.cpp constraints), then runs the overlay pass against viewportW x viewportH.
static indsin::OverlayLayer layoutAndBuildOverlay(indsin::PipelineResult& r, indsin::Indsin& engine,
                                                 double rootWidth, double viewportW, double viewportH) {
    engine.layout(r.fabric, {0, rootWidth, 0, 1e9}, 0, 0);
    return indsin::buildOverlayLayer(engine, r.fabric, viewportW, viewportH);
}
...
.end/view
        ...
        auto rc = indsin::runColdPipeline(srcClosed);
        auto ro = indsin::runColdPipeline(srcOpen);
...
.end/view
    ...
    // 4. Scrim defaults: a Dialog is modal+scrim by default; both are individually overridable.
    {
        ...
        auto def = indsin::runColdPipeline(R"(@view.Dialog=d open=true; @view.Text=t text="hi"; .end/view .end/view)");
        auto noScrim = indsin::runColdPipeline(R"(@view.Dialog=d open=true; scrim=false; @view.Text=t text="hi"; .end/view .end/view)");
        auto notModal = indsin::runColdPipeline(R"(@view.Dialog=d open=true; modal=false; @view.Text=t text="hi"; .end/view .end/view)");
        CHECK(def.ok && noScrim.ok && notModal.ok, "all three parse");
        if (def.ok && noScrim.ok && notModal.ok) {
            indsin::Indsin e1, e2, e3;
...
.end/view
        ...
        if (r.ok) {
            ...
            auto oh = indsin::hitTestOverlayLayer(layer, 5, 5);
            ...
            auto ohInside = indsin::hitTestOverlayLayer(layer, bx, by);
            ...
            indsin::TapResult tap = indsin::dispatchTapWithOverlay(r.fabric, r.warp, r.program, layer, 5, 5);
...
.end/view
        ...
        if (r.ok) {
            indsin::Indsin engine;
            auto layer = layoutAndBuildOverlay(r, engine, 390, 390, 844);
            indsin::TapResult tap = indsin::dispatchTapWithOverlay(r.fabric, r.warp, r.program, layer, 2, 2);
    ...
    //    missing-components patch already shipped and tested.
    {
        ...
        auto r = indsin::runColdPipeline(R"(@view.Tooltip=tip text="Save your work"; .end/view)");
        CHECK(r.ok, "parses");
        if (r.ok) {
            indsin::Indsin engine;
            auto layer = layoutAndBuildOverlay(r, engine, 390, 390, 844);
...
.end/view
        ...
        auto re = indsin::runColdPipeline(srcEdge);
    ...
    //    (document order == stacking order, the same convention the rest of the Indsin already uses
...
.end/view
        ...
        if (r.ok) {
            ...
            auto oh = indsin::hitTestOverlayLayer(layer, 50, 50);
