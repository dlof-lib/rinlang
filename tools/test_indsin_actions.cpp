// tools/test_indsin_actions.cpp — tests for the Indsin Actions / Container Context increment added
// alongside the professional UI upgrade work (see docs/indsintime/RIN_INDSIN_ENGINE_ARCHITECTURE.md):
//   - Needle's built-in show()/hide() Warp actions (rin_indsin_needle.h), used by
...
//     (rin_indsin_strand.h), which resolve the spec's find(id)/findByKind(kind)/parent() APIs
//     without crashing on a missing match.
//
// Build (from app/src/main/cpp):
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_actions.cpp rin_lexer.cpp rin_parser.cpp \
//       rin_interpreter.cpp rin_http.cpp diagnostics/diagnostic.cpp diagnostics/diagnostic_engine.cpp \
//       diagnostics/diagnostic_renderer.cpp diagnostics/source_manager.cpp -lz -o test_indsin_actions
...
.end/view
        ...
        if (r.ok) {
            ...
            auto res1 = indsin::dispatchTap(r.fabric, r.warp, r.program, ox, oy);
            ...
            auto res2 = indsin::dispatchTap(r.fabric, r.warp, r.program, cx, cy);
...
.end/view
        ...
        CHECK(r.ok, "parses");
        if (r.ok) {
            auto nameNode = indsin::findById(r.fabric, "name");
            CHECK(nameNode != nullptr, "findById finds an existing name");
            CHECK(indsin::findById(r.fabric, "doesNotExist") == nullptr, "findById is nil-safe for a missing name");
            CHECK(indsin::findById(nullptr, "name") == nullptr, "findById is nil-safe for a null root");

            auto btn = indsin::findByKind(r.fabric, indsin::StrandKind::BUTTON);
            CHECK(btn != nullptr && btn->name == "toggle", "findByKind finds the Button");
            CHECK(indsin::findByKind(r.fabric, indsin::StrandKind::VIDEO) == nullptr, "findByKind is nil-safe when no match exists");

            auto allText = indsin::findAllByKind(r.fabric, indsin::StrandKind::TEXT);
            CHECK(allText.size() == 1, "findAllByKind returns exactly the matching Strands");

            auto parent = indsin::findParentOf(r.fabric, nameNode->id);
            CHECK(parent != nullptr && parent->name == "profile", "findParentOf resolves the immediate parent");
            CHECK(indsin::findParentOf(r.fabric, 999999) == nullptr, "findParentOf is nil-safe for an unknown id");
            CHECK(indsin::findParentOf(r.fabric, r.fabric->id) == nullptr, "findParentOf on the root itself has no parent");
        }
    }
...
.end/view
        ...
        CHECK(r.ok, "parses");
        if (r.ok) {
            indsin::ActionRegistry& reg = indsin::ActionRegistry::shared();
...
.end/view
        ...
        if (r.ok) {
            ...
            auto res = indsin::dispatchTap(r.fabric, r.warp, r.program, btn->geometry.x + 1, btn->geometry.y + 1);
...
@view.Button=go label="Go"; onTap=seq(bogusAction(x)); .end/view
        ...
        auto rb = indsin::runColdPipeline(badSrc);
        if (rb.ok) {
            layoutAt(rb);
            auto btn = byName(rb.fabric, "go");
            auto res = indsin::dispatchTap(rb.fabric, rb.warp, rb.program, btn->geometry.x + 1, btn->geometry.y + 1);
