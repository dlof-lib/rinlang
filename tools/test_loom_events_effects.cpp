// tools/test_loom_events_effects.cpp — tests for the Events & Effects features added to Loom UI:
//   - Needle's new gesture dispatchers (rin_loom_needle.h): dispatchLongPress/dispatchDoubleTap
...
//   - The Effects Engine (rin_loom_effects.h): easing curves, per-kind EffectFrame math (fade/
...
//   g++ -std=c++17 -I. -Iloom ../../../../tools/test_loom_events_effects.cpp rin_lexer.cpp \
...
//       diagnostics/source_manager.cpp -lz -o test_loom_events_effects
...
.end/view
        ...
        auto res = loom::dispatchLongPress(r.fabric, r.warp, r.program, card->geometry.x + 5, card->geometry.y + 5);
        ...
        auto tapRes = loom::dispatchTap(r.fabric, r.warp, r.program, card->geometry.x + 5, card->geometry.y + 5);
...
@view.Button=btn label="Like"; onDoubleTap=increment(likes); .end/view
        ...
        auto res = loom::dispatchDoubleTap(r.fabric, r.warp, r.program, btn->geometry.x + 2, btn->geometry.y + 2);
...
@view.Card=card w="150"; h="60"; onHoverEnter=enter(); onHoverExit=leave(); .end/view
        ...
        auto enterRes = loom::dispatchHover(r.fabric, r.warp, r.program, px, py, /*entering=*/true);
        CHECK(enterRes.handled, "hover-enter handled");
        CHECK(r.warp.get("hovered").asString() == "true", "onHoverEnter ran");

        auto exitRes = loom::dispatchHover(r.fabric, r.warp, r.program, px, py, /*entering=*/false);
...
@view.Button=btn label="X"; state="disabled"; onLongPress=mark(); .end/view
        ...
        layoutAt(r);
        auto btn = byName(r.fabric, "btn");
        auto res = loom::dispatchLongPress(r.fabric, r.warp, r.program, btn->geometry.x + 1, btn->geometry.y + 1);
    ...
    // 5. Easing curves --------------------------------------------------------------------------
    {
        ...
        CHECK(nearlyEq(loom::applyEasing(loom::Easing::LINEAR, 0.5), 0.5), "linear(0.5) == 0.5");
        CHECK(nearlyEq(loom::applyEasing(loom::Easing::EASE_IN, 0.5), 0.25), "easeIn(0.5) == 0.25 (t*t)");
        CHECK(nearlyEq(loom::applyEasing(loom::Easing::EASE_OUT, 0.5), 0.75), "easeOut(0.5) == 0.75");
        CHECK(nearlyEq(loom::applyEasing(loom::Easing::EASE_IN_OUT, 0.25), 0.125), "easeInOut(0.25) == 0.125");
        CHECK(loom::applyEasing(loom::Easing::LINEAR, -1) == 0.0, "clamps below 0");
        CHECK(loom::applyEasing(loom::Easing::LINEAR, 2) == 1.0, "clamps above 1");
        for (double t : {0.0, 0.3, 0.7, 1.0}) {
            for (auto e : {loom::Easing::LINEAR, loom::Easing::EASE_IN, loom::Easing::EASE_OUT, loom::Easing::EASE_IN_OUT}) {
                double v = loom::applyEasing(e, t);
    ...
    // 6. EffectFrame per kind ---------------------------------------------------------------------
    {
        ...
        loom::EffectSpec spec;
        spec.easing = loom::Easing::LINEAR; // isolate the per-kind math from easing curvature

        spec.kind = loom::EffectKind::FADE;
        auto f0 = loom::evaluateEffect(spec, 0.0), f1 = loom::evaluateEffect(spec, 1.0);
        ...
        spec.kind = loom::EffectKind::SCALE;
        auto s0 = loom::evaluateEffect(spec, 0.0), s1 = loom::evaluateEffect(spec, 1.0);
        ...
        spec.kind = loom::EffectKind::SLIDE_UP;
        auto u0 = loom::evaluateEffect(spec, 0.0), u1 = loom::evaluateEffect(spec, 1.0);
        CHECK(u0.dy > 0 && nearlyEq(u1.dy, 0.0), "slideUp: starts below (positive dy), ends at 0");

        spec.kind = loom::EffectKind::SLIDE_DOWN;
        CHECK(loom::evaluateEffect(spec, 0.0).dy < 0, "slideDown: starts above (negative dy)");

        spec.kind = loom::EffectKind::SLIDE_LEFT;
        CHECK(loom::evaluateEffect(spec, 0.0).dx > 0, "slideLeft: starts to the right (positive dx)");

        spec.kind = loom::EffectKind::SLIDE_RIGHT;
        CHECK(loom::evaluateEffect(spec, 0.0).dx < 0, "slideRight: starts to the left (negative dx)");

        spec.kind = loom::EffectKind::NONE;
        auto n = loom::evaluateEffect(spec, 0.3);
    ...
    // 7. effectKindFromString / easingFromString parsing -----------------------------------------
    {
        ...
        CHECK(loom::effectKindFromString("fade") == loom::EffectKind::FADE, "\"fade\"");
        CHECK(loom::effectKindFromString("zoom") == loom::EffectKind::SCALE, "\"zoom\" aliases scale");
        CHECK(loom::effectKindFromString("slideUp") == loom::EffectKind::SLIDE_UP, "\"slideUp\"");
        CHECK(loom::effectKindFromString("slide-left") == loom::EffectKind::SLIDE_LEFT, "\"slide-left\" (dash form)");
        CHECK(loom::effectKindFromString("bogus") == loom::EffectKind::NONE, "unknown -> NONE (safe default)");
        CHECK(loom::effectKindFromString("") == loom::EffectKind::NONE, "empty -> NONE");
        CHECK(loom::easingFromString("linear") == loom::Easing::LINEAR, "\"linear\"");
        CHECK(loom::easingFromString("easeIn") == loom::Easing::EASE_IN, "\"easeIn\"");
        CHECK(loom::easingFromString("bogus") == loom::Easing::EASE_OUT, "unknown easing -> EASE_OUT default");
    }

    // 8. EffectRuntime animation clock -------------------------------------------------------------
    {
        std::cout << "-- EffectRuntime: progress over time, delay, reset on disappearance --\n";
        loom::EffectRuntime rt;
        loom::StrandId id = 42;
        ...
        loom::EffectRuntime rt3;
        loom::StrandId id3 = 99;
...
.end/view
        ...
        loom::EffectRuntime rt;
        bool animating = loom::applyEffectsToFabric(r.fabric, rt, 0.0); // first tick: t=0
        ...
        animating = loom::applyEffectsToFabric(r.fabric, rt, 100.0); // halfway
        ...
        animating = loom::applyEffectsToFabric(r.fabric, rt, 300.0); // past duration
