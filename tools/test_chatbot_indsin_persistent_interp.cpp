// tools/test_chatbot_indsin_persistent_interp.cpp — verifies the fix for the gap documented in
...
// instance (and seeded flag) into dispatchTap on every tap -- exactly how IndsinSession in
// rin_indsin_c_api.cpp now does it for every real Android/CLI session.
//
// Build (from app/src/main/cpp):
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_chatbot_indsin_persistent_interp.cpp \
...
//       -o test_chatbot_indsin_persistent_interp
...
int main() {
    ...
    auto result = indsin::runColdPipeline(kSource);
    ...
    auto tap1 = indsin::dispatchTap(result.fabric, result.warp, result.program, x, y,
    ...
    auto tap2 = indsin::dispatchTap(result.fabric, result.warp, result.program, x, y,
                                   nullptr, nullptr, &persistentInterp, &seeded);
    ...
    auto tapOld = indsin::dispatchTap(result.fabric, result.warp, result.program, x, y);
