// rin_indsin_run.cpp — Real Rin Live Preview runner.
//
// This is NOT a mock/simulator. It links directly against RIN's actual
// Lexer -> Parser -> Interpreter -> Indsintime pipeline (the same C++ engine
// the Android app calls over JNI in RinEngine.kt#renderView()) via the flat
// C ABI in indsin/rin_indsin_c_api.h. Every JSON "Fabric" this prints is produced
// by genuinely lexing, parsing, and (for taps) interpreting the .rin source —
// nothing here hand-writes or fakes the layout output.
//
// Usage:
//   rin_indsin_run render <file.rin> [rootWidth]
//       One-shot: parse + layout, print the Fabric JSON, exit.
//
//   rin_indsin_run session <file.rin> [rootWidth]
//       Interactive: keeps a live Indsintime session open (same session API
...
//   g++ -std=c++17 -O2 -o rin_indsin_run rin_indsin_run.cpp indsin/rin_indsin_c_api.cpp \
...
int cmdRender(const std::string& path, int width) {
    ...
    char* json = rin_indsin_render_json(src.c_str(), width);
...
int cmdSession(const std::string& path, int width) {
    ...
    char* initial = rin_indsin_session_render_json(session);
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (cmd == "quit" || cmd == "exit") {
            ...
            char* j = rin_indsin_session_render_json(session);
            printJson(j, "session:print");
            rin_free_string(j);
        } else if (cmd == "tap") {
            double x = 0, y = 0;
            iss >> x >> y;
            char* j = rin_indsin_session_tap(session, x, y);
            ...
            char* j = rin_indsin_session_update_source(session, newSrc.c_str());
