// tools/test_indsin_export.cpp — tests for the PNG/screenshot export engine (spec §21-23):
//   - writePNG()/rasterizeToBuffer() (rin_indsin_paint.h): a real, spec-valid PNG encoder built on
...
//   - exportPNG()/screenshot()/exportImage() wired into onTap dispatch (rin_indsin_needle.h).
//
// Build (from app/src/main/cpp):
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_export.cpp rin_lexer.cpp rin_parser.cpp \
//       rin_interpreter.cpp rin_http.cpp diagnostics/diagnostic.cpp diagnostics/diagnostic_engine.cpp \
//       diagnostics/diagnostic_renderer.cpp diagnostics/source_manager.cpp -lz -o test_indsin_export
...
int main() {
    // 1. rasterizeToBuffer / writePNG: direct unit test, no .rin source involved --------------
    {
        ...
        indsin::DrawList list;
        list.push_back({indsin::DrawOp::FILL_RECT, {0,0,10,6}, {200,10,10}, "", 0, 0, 0});
        auto buf = indsin::rasterizeToBuffer(list, 10, 6);
        CHECK(buf.size() == 10u*6u*3u, "buffer sized W*H*3");
        CHECK(buf[0] == 200 && buf[1] == 10 && buf[2] == 10, "top-left pixel matches the fill color");

        std::string path = "/tmp/indsin_export_unit_test.png";
        std::remove(path.c_str());
        bool ok = indsin::writePNG(path, 10, 6, buf);
        ...
        CHECK(!indsin::writePNG("/tmp/indsin_export_bad.png", 0, 6, buf), "zero width is rejected, not a crash");
        CHECK(!indsin::writePNG("/tmp/indsin_export_bad2.png", 10, 6, {}), "undersized buffer is rejected, not a crash");
    }
    ...
    // 2. Whole-Fabric screenshot from a real .rin source ---------------------------------------
    {
        ...
        std::string src = R"(
@view.Column=root
  @view.Button=shot label="Shot"; onTap=screenshot("/tmp/indsin_export_full.png"); .end/view
.end/view
)";
        ...
        if (r.ok) {
            ...
            std::remove("/tmp/indsin_export_full.png");
            auto res = indsin::dispatchTap(r.fabric, r.warp, r.program, x, y, nullptr, &dye);
            CHECK(res.exported && res.error.empty(), "screenshot() reports exported, no error");
            CHECK(res.exportedPath == "/tmp/indsin_export_full.png", "reports the exact path it wrote");
            auto hdr = readPngHeader("/tmp/indsin_export_full.png");
...
@view.Column=root
  ...
  .end/view
  @view.Button=shot label="Shot"; onTap=exportPNG(profile, "/tmp/indsin_export_profile.png"); .end/view
.end/view
)";
        auto r = indsin::runColdPipeline(src);
        CHECK(r.ok, "parses");
        if (r.ok) {
            ...
            auto shot = byName(r.fabric, "shot");
            double x = shot->geometry.x + 1, y = shot->geometry.y + 1;
            std::remove("/tmp/indsin_export_profile.png");
            auto res = indsin::dispatchTap(r.fabric, r.warp, r.program, x, y, nullptr, &dye);
            CHECK(res.exported && res.error.empty(), "exportPNG(container, path) dispatches without error");
            auto hdr = readPngHeader("/tmp/indsin_export_profile.png");
    ...
    // 4. Error-safety: missing Dye, unknown target, unrecognized callee --------------------------
    {
        ...
        std::string src = R"(
@view.Column=root
  @view.Button=a label="A"; onTap=exportPNG("/tmp/indsin_export_no_dye.png"); .end/view
  @view.Button=b label="B"; onTap=exportPNG(ghost, "/tmp/indsin_export_ghost.png"); .end/view
.end/view
)";
        auto r = indsin::runColdPipeline(src);
        CHECK(r.ok, "parses");
        if (r.ok) {
            ...
            auto resNoDye = indsin::dispatchTap(r.fabric, r.warp, r.program, a->geometry.x+1, a->geometry.y+1);
            ...
            auto resGhost = indsin::dispatchTap(r.fabric, r.warp, r.program, b->geometry.x+1, b->geometry.y+1, nullptr, &dye);
    ...
    // 5. Backward compatibility: old rasterizeToPPM() unaffected --------------------------------
    {
        ...
        indsin::DrawList list;
        list.push_back({indsin::DrawOp::FILL_RECT, {0,0,4,4}, {50,60,70}, "", 0, 0, 0});
        std::string path = "/tmp/indsin_export_compat.ppm";
        std::remove(path.c_str());
        indsin::rasterizeToPPM(list, 4, 4, path);
        std::ifstream f(path, std::ios::binary);
