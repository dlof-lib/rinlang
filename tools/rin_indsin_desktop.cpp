// tools/rin_indsin_desktop.cpp — Rin Indsin Desktop: a real native window for the Indsintime engine.
//
// This is the third real Indsintime host, alongside the Kotlin/Compose Android renderer
// (IndsinFabricView.kt, driven over JNI) and the CLI (rin_indsin_run.cpp, driven over stdin). It is
// NOT a mock or a re-implementation of paint/layout logic: every pixel on screen comes from
// rin_indsin_session_render_rgb(), which is the exact same Dye::paintWithOverlay() +
// rasterizeToBuffer() pass the engine's own PNG export uses (see rin_indsin_paint.h's
// "the ONE rasterizer in the engine" comment) — this tool only blits that buffer to an X11
// window with XPutImage. Taps are dispatched through rin_indsin_session_tap(), the same Needle
...
// stays as dependency-light as the rest of this repo's tools/ (matches rin_indsin_run.cpp's own
// "no external UI framework" convention, just applied to a real window instead of stdout).
//
// Usage:
//   rin_indsin_desktop window <file.rin> [rootWidth=390]
...
//
//   rin_indsin_desktop shot <file.rin> <out.png> [rootWidth=390]
...
//   g++ -std=c++17 -O2 -o rin_indsin_desktop tools/rin_indsin_desktop.cpp \
//       app/src/main/cpp/indsin/rin_indsin_c_api.cpp <...core .cpp files...> \
//       -lz -lX11 -I app/src/main/cpp

#include "../app/src/main/cpp/indsin/rin_indsin_c_api.h"
...
// ---------------------------------------------------------------------------------------------
// Headless one-shot: `rin_indsin_desktop shot <file.rin> <out.png> [rootWidth]`
// ---------------------------------------------------------------------------------------------
int cmdShot(const std::string& path, const std::string& outPng, int width) {
    ...
    void* session = rin_indsin_session_create(src.c_str(), width);
    if (!session) {
        fprintf(stderr, "error: could not create Indsintime session\n");
        return 1;
    }
    int ok = rin_indsin_session_export_png(session, outPng.c_str());
    rin_indsin_session_free(session);
    if (!ok) {
        fprintf(stderr, "error: rin_indsin_session_export_png failed (parse error in %s?)\n", path.c_str());
        return 1;
    }
...
// ---------------------------------------------------------------------------------------------
// Real interactive window: `rin_indsin_desktop window <file.rin> [rootWidth]`
// ---------------------------------------------------------------------------------------------
struct DesktopWindow {
    ...
    bool open(int width) {
        ...
        if (!display) {
            fprintf(stderr,
                ...
                "  DISPLAY=:99 ./rin_indsin_desktop window <file.rin>\n"
                "or use `rin_indsin_desktop shot <file.rin> <out.png>` for a headless PNG instead.\n",
        ...
        XStoreName(display, win, "Rin Indsin — Desktop");
    ...
    void repaint() {
        int w = 0, h = 0;
        unsigned char* rgb = rin_indsin_session_render_rgb(session, &w, &h);
        if (!rgb || w <= 0 || h <= 0) {
            if (rgb) rin_indsin_free_buffer(rgb);
    ...
    void reloadFromDisk() {
        ...
        char* j = rin_indsin_session_update_source(session, src.c_str());
    ...
    void dispatchTap(int x, int y) {
        char* j = rin_indsin_session_tap(session, (double)x, (double)y);
        if (j) rin_free_string(j); // window mode doesn't need the JSON envelope, just the repaint
        repaint();
    }
    ...
    void run(const std::string& path) {
        ...
        session = rin_indsin_session_create(src.c_str(), rootWidth);
    ...
    void close() {
        if (session) { rin_indsin_session_free(session); session = nullptr; }
...
void usage(const char* prog) {
    fprintf(stderr,
        "Rin Indsin Desktop — real X11 window for the real Indsintime engine\n"
        "usage:\n"
