// tools/test_indsin_missing_components.cpp — tests for the "missing components" pass: Grid, Wrap,
...
//   g++ -std=c++17 -I. -Iindsin ../../../../tools/test_indsin_missing_components.cpp rin_lexer.cpp \
...
//       diagnostics/source_manager.cpp -lz -o test_indsin_missing_components
...
.end/view
        ...
        if (r.ok) {
            ...
            CHECK(box->kind == indsin::StrandKind::BOX, "Box tag resolves to StrandKind::BOX");
...
.end/view
        ...
        auto r2 = indsin::runColdPipeline(src2);
        CHECK(r2.ok, "Container alias parses");
        if (r2.ok) CHECK(byName(r2.fabric, "root")->kind == indsin::StrandKind::BOX, "Container tag also resolves to StrandKind::BOX (no new StrandKind for it)");
    }
...
.end/view
        ...
        auto r1 = indsin::runColdPipeline(srcClosed);
...
.end/view
        ...
        auto r2 = indsin::runColdPipeline(srcOpen);
...
.end/view
        ...
        if (r.ok) {
            ...
            CHECK(home->kind == indsin::StrandKind::TABITEM, "TabItem resolves to its own StrandKind");
            CHECK(settings->geometry.x > home->geometry.x, "tabs lay out left-to-right like a Row");
            CHECK(indsin::resolveState(*home) == indsin::StrandState::SELECTED, "state=\"selected\" is read generically (no new state concept needed)");
        }
    }
