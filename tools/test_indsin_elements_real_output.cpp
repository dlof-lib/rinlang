// Regression coverage for real @element -> Indsin output.
#include "rin_indsin_pipeline.h"
#include <iostream>
#include <cassert>

static indsin::StrandPtr findName(const indsin::StrandPtr& s, const std::string& n) {
    if (!s) return nullptr;
    if (s->name == n) return s;
    for (auto& c : s->children) if (auto r = findName(c, n)) return r;
    return nullptr;
}
static std::string attr(const indsin::StrandPtr& s, const std::string& k) {
...
.end/loop
    ...
    indsin::Indsin engine; engine.layout(r.fabric, {0,390,0,700},0,0);
    ...
    indsin::Dye dye; auto list=dye.paint(r.fabric);
    bool buttonText=false;
    for (auto &d:list) if (d.owner==run->id && d.op==indsin::DrawOp::TEXT_RUN && d.text=="Run") buttonText=true;
